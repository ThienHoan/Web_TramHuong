...
export class AnalyticsService {
  ...
  async getDashboardData(range: string = '7d') {
    ...
    const { data: allProducts } = await this.client
      .from('products')
      .select(
        'id, category:categories(slug), translations:product_translations(title)',
      );
      ...
      if (
        ...
        if (Array.isArray(order.items)) {
          order.items.forEach((item) => {
            ...
            const pInfo = productMap[key] || {
              name: item.title || item.slug || 'Unknown',
