...
export class CategoriesService {
    ...
    async findAll(locale: string = 'en', options?: { include_inactive?: boolean; page?: number; limit?: number }) {
    ...
    async findOne(id: string) {
      ...
      `)
        ...
        if (error || !data) throw new NotFoundException(`Category with id ${id} not found`);
    ...
    async create(body: any) {
        ...
        const translations = [
            { category_id: category.id, locale: 'en', name: name_en, description: description_en },
            { category_id: category.id, locale: 'vi', name: name_vi, description: description_vi }
        ];
    ...
    async update(id: string, body: any) {
        ...
        if (body.slug || body.is_active !== undefined) {
            ...
            const { error } = await client.from('categories').update(updateData).eq('id', id);
