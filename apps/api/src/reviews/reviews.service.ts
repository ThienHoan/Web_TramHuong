...
interface ReviewWithProduct extends ReviewRecord {
...
export class ReviewsService {
  ...
  async findAllForProduct(productId: string) {
    ...
    const mappedReviews: MappedReview[] = (reviews as ReviewRecord[]).map(
      (r: ReviewRecord): MappedReview => ({
        id: r.id,
        rating: r.rating,
        comment: r.comment,
    ...
    const average =
      total > 0
        ? (mappedReviews.reduce((acc, r) => acc + r.rating, 0) / total).toFixed(
            1,
          )
        : 0;
    ...
    mappedReviews.forEach((r: MappedReview) => {
  ...
  async findAllForUser(userId: string) {
    ...
    // Map product title based on Translation if possible
    return (data as ReviewWithProduct[]).map((r: ReviewWithProduct) => ({
      ...r,
      product_title:
        r.product?.translations?.find(
          (t: ProductTranslation) => t.locale === 'en',
        )?.title || r.product?.slug,
    }));
  }
}
